const { PrismaClient, ToolStatus } = require('@prisma/client');
...
async function main() {
  ...
  const v1 = await prisma.toolVersion.upsert({
    where: { toolId_versionNumber: { toolId: seoTool.id, versionNumber: 1 } },
    update: {},
    create: {
      toolId: seoTool.id,
      versionNumber: 1,
      isPublished: true,
      webhookUrl: 'https://n8n.webhook.site/test-hook',
      inputSchema: {
        type: 'object',
        properties: {
